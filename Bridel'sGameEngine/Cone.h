...
public:
	...
	static IndexedTriangleList <V> makeTesselated(int longDiv)
	{
		...
		for (int iLong = 0; iLong < longDiv; iLong++)
		{
			...
			auto v = dx::XMVector3Transform(
				base,
				dx::XMMatrixRotationZ(longtitudeAngle * iLong)
			);
	...
	static IndexedTriangleList <V> makeTesselatedIndependentFaces(int longDiv)
	{
		...
		{
			const float thetas[] = {
				longtitudeAngle * iLong,
				longtitudeAngle * (((iLong + 1) == longDiv) ? 0 : (iLong + 1))
			};
			vertices.emplace_back();
			vertices.back().pos = { 0.0f, 0.0f, 1.0f };
		...
		{
			vertices.emplace_back();
			auto v = dx::XMVector3Transform(base, dx::XMMatrixRotationZ(longtitudeAngle * iLong));
			dx::XMStoreFloat3(&vertices.back().pos, v);
		}
